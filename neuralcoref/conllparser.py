...
def load_file(full_name, debug=False):
    ...
    with io.open(full_name, 'rt', encoding='utf-8', errors='strict') as f:
        ...
        for li, line in enumerate(lines):
            ...
            # End of utterance
            # if end of collumn, then add the story as usual
            if len(cols) == 0:
                if tokens:
    ...
    def get_feature_array(self, doc_id, feature=None, compressed=True, debug=False):
        ...
        if debug:
            ...
            for k, v in out_dict.items():
                print(k, len(v))
        del mentions_features, mentions_labels, mentions_pairs_length, mentions_pairs_start, mentions_spans, mentions_words, pairs_ant_idx, pairs_features, pairs_labels, mentions_location
