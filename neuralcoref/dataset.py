...
class NCDataset(Dataset):
    def __init__(self, data_path, params, no_targets=False):
        ...
        for file_name in os.listdir(data_path):
            ...
            print(file_name, end=', ')
            #arr = np.load(data_path + file_name)
            #dtype_used = arr.dtype
            #shape_of_array = arr.shape
            #memmap_arr = np.memmap(data_path + file_name, mode='r',dtype=dtype_used,shape=shape_of_array)
            #memmap_arr = np.load(data_path + file_name, mmap_mode='r')
            #print(":::::::::::::::::::::::::::::::::::::::")
            #print(arr.shape)
            #print(memmap_arr.shape)
            #print(":::::::::::::::::::::::::::::::::::::::")
            #del arr
            #self.datas[file_name.split(u'.')[0]] = memmap_arr #np.load(data_path + file_name)
            self.datas[file_name.split(u'.')[0]] = np.load(data_path + file_name)
        ...
        self.mentions = list(zip(*(arr for key, arr in sorted(self.datas.items()) if key.startswith(u"mentions"))))
        self.pairs = list(zip(*(arr for key, arr in sorted(self.datas.items()) if key.startswith(u"pairs"))))
        self.mentions_pair_length = self.datas[FEATURES_NAMES[2]]
        #print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        #print(FEATURES_NAMES[2])
        #print(self.datas[FEATURES_NAMES[2]])
        #print(len(self.datas[FEATURES_NAMES[2]]))
        #feat_arr = np.load(data_path + FEATURES_NAMES[2] + '.npy')
        #self.mentions_pair_length = np.load(data_path + FEATURES_NAMES[2] + '.npy')
        #print(len(feat_arr))
        #print(feat_arr)
        #print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        #rtyd = input("printing feature names ")
...
def padder_collate(batch, debug=False):
    ...
    #print("MAX PAIRS FOR ORIGINAL NEURALCOREF IS ,",max_pairs)
    #hut = input()
    if max_pairs > 0:
        out_inputs = []
        out_targets = []
