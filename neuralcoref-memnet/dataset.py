...
class NCDataset(Dataset):
    def __init__(self, data_path, params, no_targets=False):
        ...
        #print("\n\nname of data items loaded\n",datas.keys())
        #uit = input()
        self.mentions = list(zip(*(arr for key, arr in sorted(datas.items()) if key.startswith(u"mentions"))))
        #self.mentions = list()
        #self.pairs = list()
        #mentions_list_raw = list()
        #pairs_list_raw = list()
        #print("WE ARE LOADING the following")
        #for key, arr in datas.items() :
        #    if key.startswith("mentions") :
        #        print("LOADING feature#, ",key)
        #        mentions_list_raw.append(arr)
        #        print("SUCCESS IN LOADING feature#, ",key)
        #    elif key.startswith("pairs") :
        #        print("LOADING feature#, ",key)
        #        pairs_list_raw.append(arr)
        #        print("SUCCESS IN LOADING feature#, ",key)
        self.pairs = list(zip(*(arr for key, arr in sorted(datas.items()) if key.startswith(u"pairs"))))
        #self.mentions = list(zip(*tuple(mentions_list_raw)))
        #self.pairs = list(zip(*tuple(pairs_list_raw)))
        self.mentions_pair_length = datas[FEATURES_NAMES[2]]
        for arr in self.mentions[0] :
            print(arr.shape[0])

        #assert [arr.shape[0] for arr in self.mentions[0]] == [6, 1, 1, 1, 250, 8] # Cf order of FEATURES_NAMES in conllparser.py
        # in both the assert statements, 200 is the max length of story
        assert [arr.shape[0] for arr in self.mentions[0]] == [6, 1, 1, 1, 250, 200, 8] # Cf order of FEATURES_NAMES in conllparser.py
    ...
    def __getitem__(self, mention_idx, debug=False):
        ...
        """
        #print(self.mentions[mention_idx])
        #features_raw, label, pairs_length, pairs_start_index, spans, words = self.mentions[mention_idx]
        # now extracting stories from the list of items
        features_raw, label, pairs_length, pairs_start_index, spans, mentions_stories, words = self.mentions[mention_idx]
        ...
        if pairs_length == 0:
            ...
            inputs = (spans, words, features, mentions_stories)
            if self.no_targets:
                return inputs
        ...
        #assert len(pairs[0]) == 4 # pair[i] = (pairs_ant_index, pairs_features, pairs_labels,pairs_stories)
        #print(pairs[0])
        pairs_ant_index, pairs_features_raw, pairs_labels = list(zip(*pairs))

        #print(pairs_stories[0])
        ...
        # adding stories to the list of inputs
        #print(len(pairs_stories))
        #pairs_story = torch.from_numpy(pairs_stories[0])

        inputs = (spans, words, features,
                  ant_spans, ant_words,
                  ana_spans, ana_words,
                  pairs_features,mentions_stories)

        del spans, words, features, ant_spans, ant_words, ana_spans, ana_words, pairs_features, mentions_stories
...
def padder_collate(batch, debug=False):
    ...
    #print("PRINTING TRANSPOSED INPUTS\n\n",transposed_inputs)
    ...
    #print(len(transposed_inputs))
    #for item in transposed_inputs :
    #    print(item)
    #if len(transposed_inputs) == 9 :
    #    print("PRINTING TRANSPOSED INPUTS\n\n")
    #    print(transposed_inputs[3][0].size())
    ...
    #print("MAX PAIRS FOUND IS,",max_pairs)
    if max_pairs > 0:
        out_inputs = []
        out_targets = []
        for t_inp in transposed_inputs[:-1]:
        ...
        # last input is the memories
        #print("LAST INPUTS")
        #print(transposed_inputs[-1])
        out_inputs.append(torch.stack(transposed_inputs[-1],0))
    ...
    del transposed_inputs, transposed_targets
