...
def test_model(args) :
    ...
    test_evaluator = ConllEvaluator(model, test_dataset, args.test, args.testkey,
                               embed_path, args)
    ...
    def run_epochs(start_epoch, end_epoch, loss_func, optim_func, save_name, lr, g_step, debug=None):
        ...
        for epoch in tqdm(range(start_epoch, end_epoch)):
            ...
            for batch_i, (m_idx, n_pairs_l, batch) in enumerate(zip(mentions_idx, n_pairs, dataloader)):
                ...
                writer.add_scalar("train/" + save_name + "_loss", loss.data, g_step)
        ...
        writer.add_scalar("eval/" + "F1_conll", f1_conll, g_step)
        g_step += 1
        save_path = args.save_path + save_name + "_" + str(end_epoch)
        torch.save(model.state_dict(), save_path)
        load_model(model, best_model_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test', type=str, default=DIR_PATH + '/data/tiny/', help='Path to the test dataset')
    ...
    parser.add_argument('--batchsize', type=int, default=50, help='Size of a batch in total number of pairs')
    parser.add_argument('--numworkers', type=int, default=0, help='Number of workers for loading batches')
    #parser.add_argument('--numworkers', type=int, default=3, help='Number of workers for loading batches')
    ...
    parser.add_argument('--log_interval', type=int, default=10000, help='test every X mini-batches')
    ...
    #parser.add_argument('--h1', type=int, default=100, help='number of hidden unit on layer 1')
    parser.add_argument('--h2', type=int, default=500, help='number of hidden unit on layer 2')
    #parser.add_argument('--h2', type=int, default=50, help='number of hidden unit on layer 2')
    parser.add_argument('--h3', type=int, default=500, help='number of hidden unit on layer 3')
    #parser.add_argument('--h3', type=int, default=5, help='number of hidden unit on layer 3')
    parser.add_argument('--all_pairs_epoch', type=int, default=200, help='number of epochs for all-pairs pre-training')
    #parser.add_argument('--all_pairs_epoch', type=int, default=2, help='number of epochs for all-pairs pre-training')
    parser.add_argument('--top_pairs_epoch', type=int, default=200, help='number of epochs for top-pairs pre-training')
    #parser.add_argument('--top_pairs_epoch', type=int, default=2, help='number of epochs for top-pairs pre-training')
    parser.add_argument('--ranking_epoch', type=int, default=200, help='number of epochs for ranking training')
    #parser.add_argument('--ranking_epoch', type=int, default=2, help='number of epochs for ranking training')
    ...
    parser.add_argument('--train_model',dest='train_model',default=False,action='store_true', help='Is it the training phase')
    ...
    args.checkpoint_file = latest_file
