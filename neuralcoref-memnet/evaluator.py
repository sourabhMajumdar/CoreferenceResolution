...
class ConllEvaluator(object):
    ...
    def load_meta(self, test_data_path):
        ...
        print("Done")
        self.m_loc = datas[FEATURES_NAMES[10]]
        self.tokens = datas[FEATURES_NAMES[11]]
        self.lookup = datas[FEATURES_NAMES[12]]
        self.docs = datas[FEATURES_NAMES[13]]
    ...
    def get_max_score(self, batch, debug=False):
        ...
        if debug:
            ...
            max_idx = max_idx.type(torch.cuda.FloatTensor)
        #print("Executing Return Statement at get max score")
        return scores.cpu().numpy(), max_idx.cpu().numpy()
        #return scores.gpu().numpy(), max_idx.gpu().numpy()
    ...
    def get_score(self, file_path=OUT_PATH, debug=False):
        ...
        for metric_name in CONLL_METRICS:
            ...
            try:
                #print("test key file is ",self.test_key_file)
                #june = input("Enter to continue : training")
