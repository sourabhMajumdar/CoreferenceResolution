...
## Notes before preparing and training the model
There are two models to train, i.e
* Normal Neuralcoreference model from @huggingface
* Memory Augmented Neuralcoreference model proposed by us.
Both the models can be trained and using similar commands by navigating to their respective folders and following instructions as explained below.
...
````bash
python conllparser.py --path ./data/train/
python conllparser.py --path ./data/test/
python conllparser.py --path ./data/dev/
````

*Note :*
for preparing the memory augmented model you need to modify the command for the train folder by adding the flag  ``--train_phase``  i.e.
```bash
python conllparser.py --path ./data/train --train_phase
```
